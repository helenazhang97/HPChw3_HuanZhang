...
void print_results(float array[N], int tid, int section) 
{
  ...
  //#pragma omp barrier
  //Here a barrier is wrongly inserted, because some of the threads have skipped the barrier and the two threads that actually reached the barrier still wait here forever. And the program just hangs. So I commented that out and everything seems to be working fine.
